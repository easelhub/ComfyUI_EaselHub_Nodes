...
app.registerExtension({
    name: "EHN.SetNode",
    registerCustomNodes() {
        class EHN_SetNode extends LGraphNode {
            ...
            constructor(title) {
                ...
                this.properties.showOutputText = EHN_SetNode.defaultVisibility;
                ...
                this.validateName = function(graph) {
                    ...
                    if (widgetValue !== '') {
                        ...
                        graph._nodes.forEach(otherNode => {
                            if (otherNode !== this && otherNode.type === 'EHN_SetNode') {
                ...
                this.clone = function () {
                    const cloned = EHN_SetNode.prototype.clone.apply(this);
                ...
                this.update = function() {
                    ...
                    const allGetters = node.graph._nodes.filter(otherNode => otherNode.type === "EHN_GetNode");
                ...
                this.findGetters = function(graph, checkForPreviousName) {
                    ...
                    return graph._nodes.filter(otherNode => otherNode.type === 'EHN_GetNode' && otherNode.widgets[0].value === name && name !== '');
                };
            ...
            onRemoved() {
                const allGetters = this.graph._nodes.filter((otherNode) => otherNode.type == "EHN_GetNode");
                allGetters.forEach((otherNode) => {
                    if (otherNode.setComboValues) {
            ...
            getExtraMenuOptions(_, options) {
                ...
                options.unshift(
                    ...
                    },
                    {
                        ...
                        callback: () => {
                            const allGetters = this.graph._nodes.filter(otherNode => otherNode.type === "EHN_GetNode" || otherNode.type === "EHN_SetNode");
                            allGetters.forEach(otherNode => {
            ...
            _drawVirtualLinks(lGraphCanvas, ctx) {
                ...
                for (const getter of this.currentGetters) {
                    var end_node_slotpos;
                    if (!this.flags.collapsed) {
                        end_node_slotpos = this.getConnectionPos(false, 0);
                        end_node_slotpos = [
        ...
        LiteGraph.registerNodeType(
            "EHN_SetNode",
            Object.assign(EHN_SetNode, {
                title: "Set",
            })
        );

        EHN_SetNode.category = "EaselHub/Utils";
    },
});
...
app.registerExtension({
    name: "EHN.GetNode",
    registerCustomNodes() {
        class EHN_GetNode extends LGraphNode {
            ...
            constructor(title) {
                ...
                this.properties.showOutputText = EHN_GetNode.defaultVisibility;
                const node = this;
                this.addWidget(
                    "combo",
                    "Constant",
                    ...
                    {
                        values: () => {
                            const setterNodes = node.graph._nodes.filter((otherNode) => otherNode.type == 'EHN_SetNode');
                ...
                this.clone = function () {
                    const cloned = EHN_GetNode.prototype.clone.apply(this);
                    cloned.size = cloned.computeSize();
                    return cloned;
                };
                ...
                this.findSetter = function(graph) {
                    ...
                    const foundNode = graph._nodes.find(otherNode => otherNode.type === 'EHN_SetNode' && otherNode.widgets[0].value === name && name !== '');
                    return foundNode;
                };
            ...
            getInputLink(slot) {
                ...
                if (setter) {
                    ...
                    const errorMessage = "No EHN_SetNode found for " + this.widgets[0].value + "(" + this.type + ")";
        ...
        LiteGraph.registerNodeType(
            "EHN_GetNode",
            Object.assign(EHN_GetNode, {
                title: "Get",
            })
        );

        EHN_GetNode.category = "EaselHub/Utils";
    },
});
