...
class EHN_ImageResize:
    @classmethod
    def INPUT_TYPES(s):
        return {"required": {"image": ("IMAGE",), "width": ("INT", {"default": 1024, "min": 0, "max": 8192, "step": 8}), "height": ("INT", {"default": 1024, "min": 0, "max": 8192, "step": 8}), "interpolation": (["nearest", "bilinear", "bicubic", "area"], {"default": "bicubic"}), "method": (["stretch", "keep proportion", "fill / crop", "pad"], {"default": "keep proportion"}), "condition": (["always", "downscale if bigger", "upscale if smaller", "if bigger area", "if smaller area"], {"default": "always"}), "multiple_of": ("INT", {"default": 8, "min": 0, "max": 512, "step": 8}), "mask_expansion": ("INT", {"default": 0, "min": -128, "max": 128, "step": 1}), "mask_blur": ("INT", {"default": 0, "min": 0, "max": 64, "step": 1}), "mask_fill_holes": ("BOOLEAN", {"default": False}), "mask_invert": ("BOOLEAN", {"default": False})}, "optional": {"mask": ("MASK",)}}
    ...
    def execute(self, image, width, height, interpolation, method, condition, multiple_of, mask_expansion, mask_blur, mask_fill_holes, mask_invert, mask=None):
        ...
        if condition == "downscale if bigger":
            ...
            if W * H >= target_width * target_height: target_width, target_height = W, H
        if method in ["keep proportion", "pad"]:
