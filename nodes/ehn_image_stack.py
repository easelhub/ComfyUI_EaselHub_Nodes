...
class EHN_ImageStack:
    ...
    def execute_stack(self, image_1, layout_mode, max_columns, resize_behavior, resize_value, alignment, gap, border_width, fill_color, fill_holes, mask_blur, mask_1=None, **kwargs):
        ...
        out_img = torch.tensor(bg, device=p_imgs[0].device).view(1,1,3).repeat(total_h, total_w, 1)
        out_msk = torch.zeros((total_h, total_w), device=p_imgs[0].device)
        cy = bw
        for r_idx, r_imgs in enumerate(rows):
            rh, rw = row_dims[r_idx]
            cx = bw
            if "center" in align.lower(): cx += (total_w - 2*bw - rw)//2
            elif "right" in align.lower(): cx += (total_w - 2*bw - rw)
            for img, msk in zip(r_imgs, rows[r_idx]):
                if img is None: continue
                iy = cy + (rh - img.shape[0])//2
                out_img[iy:iy+img.shape[0], cx:cx+img.shape[1]] = img
                out_msk[iy:iy+img.shape[0], cx:cx+img.shape[1]] = p_msks[p_imgs.index(img)]
                cx += img.shape[1] + gp
            cy += rh + gp
        return (out_img.unsqueeze(0), out_msk.unsqueeze(0), total_w, total_h)
