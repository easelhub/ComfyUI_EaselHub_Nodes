...
class EHN_LLM_Base:
    ...
    def _types(models, key_def="sk-...", extra=None):
        ...
        opt = {"custom_model_name":("STRING",{"default":""}), "language":(["English","Chinese"],), "seed":("INT",{"default":0})}
        if extra: opt.update(extra)
        return {"required": req, "optional": opt}

    def _exec(self, url, key, model, usr, sys, lang, seed, headers=None, payload=None, method="post"):
        ...
        try:
            r = requests.post(url, headers=h, json=p, timeout=60) if method=="post" else requests.get(url, headers=h, params=p, timeout=60)
            r.raise_for_status()
            d = r.json()
...
class EHN_SiliconFlow(EHN_LLM_Base):
    ...
    def generate_prompt(self, api_key, model, user_prompt, system_prompt, seed, custom_model_name="", language="English"):
        return self._exec("https://api.siliconflow.cn/v1/chat/completions", api_key, custom_model_name or model, user_prompt, system_prompt, language, seed)
...
class EHN_OpenRouter(EHN_LLM_Base):
    ...
    def generate_prompt(self, api_key, model, user_prompt, system_prompt, seed, custom_model_name="", language="English"):
        ...
        return self._exec("https://openrouter.ai/api/v1/chat/completions", api_key, "", "", "", "", seed, {"HTTP-Referer":"https://github.com","X-Title":"EaselHub"}, p)

class EHN_DeepSeek(EHN_LLM_Base):
    @classmethod
    def INPUT_TYPES(s): return s._types((["deepseek-chat", "deepseek-reasoner"],))
    def generate_prompt(self, api_key, model, user_prompt, system_prompt, seed, custom_model_name="", language="English"):
        return self._exec("https://api.deepseek.com/chat/completions", api_key, custom_model_name or model, user_prompt, system_prompt, language, seed)
...
class EHN_OpenAI(EHN_LLM_Base):
    ...
    def generate_prompt(self, api_key, model, user_prompt, system_prompt, seed, custom_model_name="", language="English", base_url="https://api.openai.com/v1"):
        u = base_url.rstrip("/") + ("/chat/completions" if not base_url.endswith("/chat/completions") else "")
        return self._exec(u, api_key, custom_model_name or model, user_prompt, system_prompt, language, seed)
...
class EHN_CustomLLM(EHN_LLM_Base):
    @classmethod
    def INPUT_TYPES(s): return {"required":{"base_url":("STRING",{"default":"http://localhost:11434/v1"}), "api_key":("STRING",{"default":"sk-..."}), "model":("STRING",{"default":"llama3"}), "system_prompt":("STRING",{"multiline":True,"default":SYSTEM_PROMPT}), "user_prompt":("STRING",{"multiline":True})}, "optional":{"language":(["English","Chinese"],), "seed":("INT",{"default":0})}}
    def generate_prompt(self, base_url, api_key, model, user_prompt, system_prompt, seed, language="English"):
        u = base_url.rstrip("/") + ("/chat/completions" if not base_url.endswith("/chat/completions") else "")
        return self._exec(u, "ollama" if api_key=="sk-..." else api_key, model, user_prompt, system_prompt, language, seed)